...
function run_one(data, name)
    ...
    mgfpt10 = update(data, mgfpt10)
    mgfpt10_ll = parallel_log_lik(data, mgfpt10)
    waic_mgfpt10 = compute_waic(mgfpt10_ll)
    ...
    mgfpt2 = update(data, mgfpt2)
    mgfpt2_ll = parallel_log_lik(data, mgfpt2)
    waic_mgfpt2 = compute_waic(mgfpt2_ll)

    println("waic_mgfpt10: ", waic_mgfpt10, ", waic_mgfpt2: ", waic_mgfpt2)
    ...
    return waic_mgfpt10, waic_mgfpt2
end
...
function main()
    ...
    out = run_one(data, "eurodist")
    rows = push!(rows, ("eurodist",  out... ))

    println("GDP")
    data = read_data("data/gdp.csv")
    out  = run_one(data, "gdp")
    rows = push!(rows, ("gdp",  out... ))

    println("CENSUS")
    data = read_data("data/census.csv")
    out = run_one(data, "census")
    rows = push!(rows, ("census",  out... ))

    println("TWITTER")
    data = read_data("data/twitter_friends.csv")
    out  = run_one(data, "twitter")
    rows = push!(rows, ("twitter",  out... ))

    println("INCOME")
    data = read_data("data/income.csv")
    out  = run_one(data, "income")
    rows = push!(rows, ("income",  out... ))

    df = DataFrame(rows)
    CSV.write("data/real_data_lavine_alphas.csv", df)
end
